...
export class RunContext {
  ...
  /**
   * Create a forked RunContext for a sub-agent run.
   * The fork gets fresh tracking state but inherits conversation context.
   */
  fork(overrides?: { userQuery?: string; conversationId?: string }): RunContext {
    return new RunContext({
      userQuery: overrides?.userQuery ?? this.userQuery,
