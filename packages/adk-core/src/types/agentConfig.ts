import type { ResponsesApiInputItem } from './responsesApi';
...
export interface HandoffInputFilterContext {
  fromAgentName: string;
  toAgentName: string;
  reason?: string;
}

export type HandoffInputFilterFunction = (
...
export type ToolUseBehaviorFunction = (
  context: ToolUseBehaviorContext,
  toolResults: ToolUseBehaviorToolResult[],
) => ToolUseBehaviorDecision;

export interface ToolUseBehaviorContext {
  agentName: string;
  agentKey: string;
  turn: number;
}

export interface ToolUseBehaviorToolResult {
  toolName: string;
  callId: string;
  output: string;
  error?: string;
...
export interface ToolUseBehaviorDecision {
